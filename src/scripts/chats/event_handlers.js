...
Chats.prototype.handle_click_create = async function(user_id, user_name, first_name, last_name, extension) {
	this.ls.classList.add('d-none')
	let {status, chat_id, row_down} = await fetchs(`chats/create`, {user_id2: user_id})   //`user_id=${user_id}`
	...
	socket[this.names].emit('new-chat', {'chat_id': chat_id})
...
Chats.prototype.handle_click_leave = async function() {
...
Chats.prototype.handle_focus_text_message = async function() {
	document.addEventListener("keyup", chats.handle_keyup_send_message)
}
Chats.prototype.handle_blur_text_message = async function() {
